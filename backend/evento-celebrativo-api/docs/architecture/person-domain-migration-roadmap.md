...
- O profile `local` define `app.event-assignment.read-source.monthly-schedule=${EVENT_ASSIGNMENT_READ_SOURCE_MONTHLY_SCHEDULE:PARALLEL}` para validar a consulta mensal por `tb_event_assignment`; base, `test` e `mysql` permanecem em `LEGACY` salvo override explicito.
