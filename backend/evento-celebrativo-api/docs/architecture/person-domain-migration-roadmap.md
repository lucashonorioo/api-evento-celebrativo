...
| 6. Migrar escalas | Fase 5 | `event_assignment` preenchida | Em andamento: escrita em compatibilidade, backfill concluido, leitura paralela interna, auditoria e shadow read configuravel disponiveis; endpoints ainda legados |
...
3. Ativar o shadow read de assignments de forma controlada no profile `local` e acompanhar estabilidade antes de qualquer cutover.
...
- O shadow read de `EventAssignment` e configuravel por fluxo com as flags `event-detail-enabled`, `event-scale-detail-enabled`, `monthly-schedule-enabled` e `eucharist-scale-enabled`, todas desabilitadas por padrao em todos os profiles.
...
- A proxima etapa de escalas sera ativar o shadow read controlado no profile `local` e executar testes de estabilidade antes de qualquer cutover de leitura.
