...
- O dual write permanece ativo sem alteracao: a escrita legada (`CelebrationEvent.people`/`tb_event_person`) continua sendo a primeira escrita da transacao, e `EventAssignmentTargetResolver`/`EventAssignmentCompatibilityService` continuam sincronizando `tb_event_assignment` a partir das pessoas ja validadas, sem consultar `PersonMinistry`.
...
- A proxima etapa planejada e preparar a escrita oficial de escala baseada em `EventAssignment` como fonte primaria (hoje a escrita legada ainda e a primeira a acontecer na transacao) e, apos periodo de estabilidade, remover gradualmente a restricao de representabilidade legada mantida por `ScaleLegacyCompatibilityValidator`.
