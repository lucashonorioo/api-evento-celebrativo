...
- Os profiles `local` e `mysql` definem `app.event-assignment.read-source.event-scale-detail=${EVENT_ASSIGNMENT_READ_SOURCE_EVENT_SCALE_DETAIL:PARALLEL}` para validar o detalhe da escala por `tb_event_assignment`; base e `test` permanecem em `LEGACY` salvo override explicito.
...
- Os profiles `local` e `mysql` definem `app.event-assignment.read-source.eucharist-scale=${EVENT_ASSIGNMENT_READ_SOURCE_EUCHARIST_SCALE:PARALLEL}` para validar a escala eucaristica por `tb_event_assignment`; base e `test` permanecem em `LEGACY` salvo override explicito.
...
- `GET /eventos/escalas` possui origem oficial configuravel por `app.event-assignment.read-source.monthly-schedule`, com valores `LEGACY` e `PARALLEL`; o default global continua `LEGACY`.
- Os profiles `local` e `mysql` definem `app.event-assignment.read-source.monthly-schedule=${EVENT_ASSIGNMENT_READ_SOURCE_MONTHLY_SCHEDULE:PARALLEL}` para validar a consulta mensal por `tb_event_assignment`; base e `test` permanecem em `LEGACY` salvo override explicito.
...
- A validacao consolidada dos tres cutovers de escala (`GET /eventos/{id}/escala`, `GET /eventos/escala/eucaristia` e `GET /eventos/escalas`) confirma que os profiles `local` e `mysql` podem executar os tres em `PARALLEL` simultaneamente, enquanto base e `test` permanecem em `LEGACY` por padrao.
...
- `tb_event_person` permanece preservada e nenhuma estrutura legada foi removida; a proxima etapa sera manter um periodo de estabilidade antes de avaliar mudanca do default global.
