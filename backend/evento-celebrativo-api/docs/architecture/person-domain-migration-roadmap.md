...
| 6. Migrar escalas | Fase 5 | `event_assignment` preenchida | Em andamento: escrita em compatibilidade, backfill concluido, leitura paralela interna, auditoria, shadow read configuravel e cutover controlado do detalhe da escala disponiveis; demais endpoints ainda legados |
...
4. Observar a estabilidade do primeiro cutover controlado de detalhe da escala antes de expandir para outras leituras de escala.
...
- Endpoints, DTOs, mappers de resposta, consulta mensal, escala eucaristica e listagem geral continuam com contratos HTTP inalterados; consulta mensal, escala eucaristica e listagem geral de eventos continuam usando o modelo legado.
- A proxima etapa devera observar a estabilidade do cutover do detalhe da escala e avaliar as limitacoes das consultas parciais antes de preparar novas leituras oficiais por `tb_event_assignment`.
