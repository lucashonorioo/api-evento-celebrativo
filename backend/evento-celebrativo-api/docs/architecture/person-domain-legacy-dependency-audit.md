...
| --- | --- | --- |
| `ReaderRepository`, `CommentatorRepository`, `PriestRepository`, `MinisterOfTheWordRepository`, `EucharisticMinisterRepository` | sem consumidor de producao desde `refactor/make-person-ministry-writes-official` (os cinco CRUDs passaram a usar `PersonRepository`/`PersonMinistryCommandService`); ainda usados por fixtures/asserts de testes historicos | `NO_PRODUCTION_CONSUMER`. Mantidos por serem necessarios a heranca JPA `SINGLE_TABLE` (instanciacao das subclasses) e por `HISTORICAL_TEST_ONLY`. |
| `PersonMinistryCompatibilityService`/`PersonMinistryCompatibilityServiceImpl` | sem consumidor de producao desde `refactor/make-person-ministry-writes-official` (os cinco CRUDs passaram a usar `PersonMinistryCommandService`); ainda usado por `PersonMinistryCompatibilityServiceImplTest` | `OBSOLETE_AFTER_OFFICIAL_WRITES`. Nao removido nesta branch para nao ampliar o escopo; candidato a remocao na proxima etapa de limpeza. |
