...
> **Estado final (2026-07-27, `refactor/finalize-person-domain-cleanup`): AUDITORIA ENCERRADA — nenhuma dependencia legada atual restante.**
> As secoes 2 a 18 abaixo (tabelas e analises) descrevem o estado do codigo em **2026-07-24** e foram supersedidas por uma serie de branches de remocao ja concluidas (listadas nos paragrafos de atualizacao da secao 1, e detalhadas em [`person-domain-migration-roadmap.md`](person-domain-migration-roadmap.md)). Em particular: `person_type` e `tb_event_person` foram fisicamente removidos (`V7`, `V8`); as cinco subclasses ministeriais de `Person` e a heranca JPA `SINGLE_TABLE` foram removidas; `PersonMinistryCompatibilityService`, os cinco repositories de subtipo, `MinistryTypeResolver`, `PersonMinistryConsistencyService`, todas as propriedades `*.read-source.*`/`*.shadow-read.*-enabled` e os cutovers `LEGACY`/`PARALLEL` foram removidos por completo; e o campo `personType` do contrato administrativo (`GET/PUT /pessoas`, `PUT /pessoas/{id}/roles`) foi substituido por `ministries: List<MinistryType>`. Uma nova auditoria completa por grep dirigido (`src/main`, `src/test`, migrations, seeds, documentacao), executada nesta branch, confirmou **zero** ocorrencia `ACTIVE_CURRENT_RUNTIME` de qualquer termo legado listado nas secoes abaixo. Nenhuma tabela ou paragrafo das secoes 2-18 deve ser tratado como descricao do codigo atual; trate-as como registro historico. Nesta mesma branch, `EventAssignmentCompatibilityService`/`EventAssignmentCompatibilityServiceImpl` foi renomeado para `EventAssignmentCommandService`/`EventAssignmentCommandServiceImpl` (rename puro, sem mudanca de comportamento ou contrato); onde as secoes 2 e 12 abaixo (linhas sobre "Compatibilidade de escrita" e sobre `EventAssignmentCompatibilityService`) citam o nome antigo, leia-se `EventAssignmentCommandService`.
...
Atualizacao posterior adicional (`refactor/make-person-ministry-writes-official`, 2026-07-26): `Person` (via `PersonRepository`) e `PersonMinistry` (via `PersonMinistryRepository`) passaram a ser a fonte oficial de escrita dos cinco CRUDs ministeriais, atraves do novo `PersonMinistryCommandService`/`PersonMinistryCommandServiceImpl`. Pontos de escrita migrados: criacao (`create`), busca individual (`requireActiveMinistryPerson`, tambem usado como pre-condicao do `update`), atualizacao (`save`, apos o mapper aplicar o DTO sobre a `Person` ja carregada) e exclusao ministerial (`removeMinistry`, que desativa apenas o vinculo solicitado — nunca apaga a `Person` — e verifica conflito com `EventAssignment` do mesmo tipo antes de desativar). Nenhum dos quatro metodos usa `instanceof`, `person_type` ou repository de subtipo; a classificacao ministerial e sempre o `MinistryType` explicito informado pelo service de origem. Multiplos ministerios da mesma pessoa passaram a ser preservados em update/delete (antes, excluir por qualquer um dos cinco endpoints apagava a `Person` inteira via `deleteById` do repository de subtipo, o que e incompativel com uma pessoa que possui mais de um `PersonMinistry` ativo); subtipo legado divergente do `PersonMinistry` solicitado deixou de bloquear `findById`/`update` (antes, os repositories de subtipo restringiam a busca pelo discriminator `person_type`, entao uma pessoa com `person_type` diferente do ministerio ativo simplesmente nao era encontrada). Limitacao transitoria de JPA que permanece: `Person` continua abstrata (heranca `SINGLE_TABLE`), entao a criacao ainda precisa instanciar uma das cinco subclasses concretas — essa escolha e feita apenas pelo mapper do service de origem (nunca pelo `PersonMinistryCommandService`, que so recebe `Person` + `MinistryType` explicito) e nao influencia a classificacao ministerial em nenhum ponto de leitura, busca, atualizacao ou exclusao. Os detalhes completos estao no roadmap, secao "Migracao dos CRUDs ministeriais para escrita oficial por Person + PersonMinistry (2026-07-26)".

Atualizacao posterior adicional (`refactor/remove-obsolete-person-ministry-write-through`, 2026-07-26): `PersonMinistryCompatibilityService`/`PersonMinistryCompatibilityServiceImpl` (`ensureMinistry`/`deleteAllForPerson`), classificados na atualizacao anterior como `OBSOLETE_AFTER_OFFICIAL_WRITES`, foram removidos por completo (interface, implementacao e `PersonMinistryCompatibilityServiceImplTest`), confirmada a ausencia de qualquer outro consumidor. Os cinco repositories de subtipo (`ReaderRepository`, `CommentatorRepository`, `PriestRepository`, `MinisterOfTheWordRepository`, `EucharisticMinisterRepository`), classificados como `NO_PRODUCTION_CONSUMER`/`HISTORICAL_TEST_ONLY`, tambem foram removidos por completo: os 11 arquivos de teste historico que ainda os usavam (`findAll()`/`existsById()` para montar uma "visao legada" por `person_type`) foram adaptados para `JdbcTemplate` (consulta direta a `tb_person` por `person_type`) ou `PersonRepository.findAllByIdIn(ids)` quando o teste precisava de entidades `Person` completas. A remocao das cinco interfaces nao exigiu remover as subclasses (`Reader`, `Commentator`, `Priest`, `MinisterOfTheWord`, `EucharisticMinister` continuam existindo como entidades JPA, instanciadas pelos mappers de cada service em `create()`), nem alterar a heranca `SINGLE_TABLE`, o discriminator `person_type` ou a migration `V4` (backfill), que permanecem exatamente como antes. `Person` continua abstrata — a limitacao transitoria de JPA documentada na atualizacao anterior nao mudou. Os detalhes completos estao no roadmap, secao "Remocao da infraestrutura obsoleta de write-through de PersonMinistry (2026-07-26)".

Atualizacao posterior adicional (`refactor/remove-legacy-person-inheritance`, 2026-07-26): `person_type`, a heranca JPA `SINGLE_TABLE` de `Person` e as cinco subclasses ministeriais legadas — os tres itens que a secao 4 abaixo classificava como `CONTRACT_REQUIRED`/dependencia ativa mais critica — foram removidos por completo. `Person` (`model/Person.java`) deixou de ser `abstract`: `@Inheritance`, `@DiscriminatorColumn(name = "person_type")`, o campo `personType` e o construtor de seis argumentos foram removidos, e os seis metodos de `UserDetails` antes duplicados nas cinco subclasses foram consolidados diretamente na classe. `Reader`, `Commentator`, `Priest`, `MinisterOfTheWord` e `EucharisticMinister` foram apagadas; os cinco services especificos, controllers e DTOs nao mudaram (permanecem como adaptadores dos contratos HTTP), e os cinco `*ServiceImpl` so tiveram o tipo da variavel local do mapper trocado para `Person`. `MinistryTypeResolver` (100% `instanceof`) e `PersonMinistryConsistencyService`/`Impl` (classificados na secao 13 como `LEGACY_COMPATIBILITY`) foram removidos: a auditoria interna existia para comparar "ministerio esperado pelo subtipo Java" com `PersonMinistry`, uma comparacao cuja premissa desaparece junto do discriminator. Migration `V8__remove_legacy_person_discriminator` (Java, modelada no padrao de `V7`) valida que toda pessoa possui `PersonMinistry` ativo correspondente ao `person_type` historico antes de `ALTER TABLE tb_person DROP COLUMN person_type` (sintaxe identica em H2 e MySQL 8.4, sem dialeto especial); a migration falha sem alterar dados quando encontra divergencia. O campo e o filtro `personType` do endpoint administrativo `GET/PUT /pessoas` foram preservados no contrato HTTP, mas passaram a ser projetados a partir de `PersonMinistry` (`PersonMinistryReadService.findActiveMinistriesByPersonIds`), com uma politica deterministica documentada para pessoas com multiplos ministerios ativos (menor ordinal do enum `MinistryType` vence) — uma limitacao intencional do novo modelo, nao uma ambiguidade nao resolvida. Validado em H2 (475 -> 467 testes, 0 falhas/erros; a reducao vem da remocao das duas classes de auditoria interna e de metodos de teste cuja premissa deixou de existir, compensada por 3 testes novos de migration) e em MySQL 8.4 real (Flyway `V1`-`V8`, `ddl-auto=validate`, suite direcionada de 21 testes cobrindo os cinco CRUDs, as cinco listagens, multiplos ministerios e conflito de `EventAssignment`, 0 falhas/erros). Os detalhes completos estao no roadmap, secao "Remocao da heranca JPA legada de Person e do discriminator person_type (2026-07-26)".

Atualizacao posterior adicional (`feature/person-ministry-management`, 2026-07-27): esta branch nao remove nenhuma dependencia legada adicional; adiciona a primeira API administrativa dedicada a gerenciar o conjunto completo de ministerios de uma pessoa existente (`GET`/`PUT /pessoas/{id}/ministries`), reaproveitando `PersonMinistryCommandService` (ampliado com `syncMinistries`), `PersonRepository`, `PersonMinistryRepository` e `EventAssignmentRepository` ja existentes, sem tocar nos cinco CRUDs ministeriais, em `EventAssignment`, em autenticacao/roles ou nas migrations `V1`-`V8`. O ponto relevante para esta auditoria: o contrato administrativo `personType` (singular, secao 4 abaixo) permanece exatamente como estava — nao foi removido nem alterado — mas agora coexiste com um contrato oficial plural (`ministries`) que expõe a mesma fonte (`PersonMinistry`) sem a limitacao de "uma unica funcao por resposta". Isso muda a avaliacao de risco de uma eventual remocao futura de `personType`: antes, remove-lo exigia decidir um substituto; agora o substituto (`ministries`) ja existe e esta em producao, o que reduz o risco tecnico de uma futura remocao de `personType` a uma decisao de compatibilidade com o frontend, nao mais de design de API. Os detalhes completos estao no roadmap, secao "Gestao administrativa de ministerios de uma pessoa existente (2026-07-27)".

Atualizacao posterior adicional (`refactor/replace-person-type-with-ministries-contract`, 2026-07-27): as tres linhas da tabela da secao 4 relativas a `personType` (`Discriminator JPA`/campo `personType`, `PersonAdminResponseDTO.personType` e o filtro administrativo por `ALLOWED_PERSON_TYPES`) e a linha correspondente da secao 12 estao **resolvidas/obsoletas** a partir desta branch, permanecendo abaixo apenas como registro historico do estado em 2026-07-24. `personType` foi removido dos dois DTOs administrativos que ainda o expunham (`PersonAdminResponseDTO`, usado por `GET /pessoas`/`GET /pessoas/{id}`; e `PersonRoleUpdateResponseDTO`, usado por `PUT /pessoas/{id}/roles`), substituido por `ministries: List<MinistryType>` nos dois. A projecao pelo menor ordinal do enum (unica logica que ainda tratava "uma pessoa, um tipo" em todo o codebase) foi removida por completo. O filtro de query `personType` de `GET /pessoas` foi renomeado para `ministry`, validado agora diretamente contra o enum `MinistryType` (sem a lista fixa `ALLOWED_PERSON_TYPES`, que foi removida). Nenhuma nova dependencia legada foi criada; `PersonMinistryReadService.findActiveMinistriesByPersonIds` (ja existente, em lote) continua sendo a unica fonte de leitura, agora compartilhada sem excecao entre a resposta administrativa geral e o endpoint especifico `GET`/`PUT /pessoas/{id}/ministries`. O frontend (`admin-users/admin-user.models.ts`, `admin-user.service.ts`, `admin-user-management.component.ts`/`.html`, e os respectivos `.spec.ts`) ainda consome `personType` e quebra contra este backend ate ser migrado para `ministries`/`ministry` numa branch frontend separada — o consumo de frontend citado na linha 120 da secao 4 deixou de ser um motivo para manter `personType` no backend (o contrato ja mudou de forma intencional) e passou a ser, em vez disso, o proximo trabalho pendente. Os detalhes completos estao no roadmap, secao "Remocao de personType do contrato administrativo de pessoas (2026-07-27)".

Apesar disso, o modelo legado ainda e uma dependencia ativa. A remocao imediata de `tb_event_person` (ja removida fisicamente, ver acima) nao e segura; `person_type` e as subclasses de `Person` ja foram removidos (ver atualizacao acima).
...
- (resolvido em `refactor/remove-legacy-person-inheritance`) a validacao de escala ja nao dependia de subtipo Java, e os CRUDs ministeriais deixaram de criar subclasses concretas; `Person` e concreta;
- (resolvido em `refactor/remove-legacy-person-inheritance`) os CRUDs ministeriais nao criam mais subclasses concretas; `Reader`, `Commentator`, `Priest`, `MinisterOfTheWord` e `EucharisticMinister` foram removidas;
- (resolvido em `refactor/replace-person-type-with-ministries-contract`) a administracao de usuarios deixou de expor e filtrar `personType`; o contrato HTTP passou a expor `ministries: List<MinistryType>` e o filtro `ministry`, com o frontend ainda pendente de migracao (ver atualizacao acima);
- as leituras `LEGACY`, shadow read, auditorias e testes de rollback de `EventAssignment`/`PersonMinistry` (fora do escopo desta branch) ainda podem depender de decisoes historicas do legado;
- `V4` e `V5` continuam sendo migrations ja aplicadas com `person_type`/`tb_event_person` como fonte historica de backfill (registro imutavel; a coluna `person_type` em si foi removida por `V8`);
...
| --- | --- | --- | --- | --- | --- | --- |
| `Reader` | `Reader` (repository de subtipo removido em `refactor/remove-obsolete-person-ministry-write-through`) | `ReaderServiceImpl`, `ReaderController`, `ReaderRequestDTO`, `ReaderResponseDTO`, `ReaderMapper` | `/leitores` | `PersonMinistryCommandService` cria/gerencia `READER` diretamente (sem write-through); listagem usa `tb_person_ministry` | `Person + ministries` com contrato unificado | Tela de leitores pode manter contrato atual se backend adaptar internamente; API unificada exigiria mudanca. |
| `Commentator` | `Commentator` (repository de subtipo removido) | `CommentatorServiceImpl`, `CommentatorController`, DTOs/mappers | `/comentaristas` | `PersonMinistryCommandService` cria/gerencia `COMMENTATOR` diretamente; listagem usa `tb_person_ministry` | `Person + ministries` | Mesmo impacto de leitores. |
| `Priest` | `Priest` (repository de subtipo removido) | `PriestServiceImpl`, `PriestController`, DTOs/mappers | `/padres` | `PersonMinistryCommandService` cria/gerencia `PRIEST` diretamente; listagem usa `tb_person_ministry` | `Person + ministries` | Edicao de escalas usa lista de padres; precisa continuar retornando `id/name` compativel. |
| `MinisterOfTheWord` | `MinisterOfTheWord` (repository de subtipo removido) | `MinisterOfTheWordServiceImpl`, controller, DTOs/mappers | `/ministrosDaPalavra` | `PersonMinistryCommandService` cria/gerencia `MINISTER_OF_THE_WORD` diretamente; listagem usa `tb_person_ministry` | `Person + ministries` | Edicao de escalas usa lista atual; contrato pode ser preservado. |
| `EucharisticMinister` | `EucharisticMinister` (repository de subtipo removido) | `EucharisticMinisterServiceImpl`, controller, DTOs/mappers | `/ministrosDeEucaristia` | `PersonMinistryCommandService` cria/gerencia `EUCHARISTIC_MINISTER` diretamente; listagem usa `tb_person_ministry` | `Person + ministries` | Publico e admin de ministros da Eucaristia dependem do contrato atual. |
...
| --- | --- | --- |
| ~~`ReaderRepository`, `CommentatorRepository`, `PriestRepository`, `MinisterOfTheWordRepository`, `EucharisticMinisterRepository`~~ | sem consumidor de producao desde `refactor/make-person-ministry-writes-official`; o uso remanescente em 11 testes historicos foi migrado para `JdbcTemplate`/`PersonRepository.findAllByIdIn` | Removidas por completo em `refactor/remove-obsolete-person-ministry-write-through`. Subclasses, discriminator e heranca `SINGLE_TABLE` preservados. |
| ~~`PersonMinistryCompatibilityService`/`PersonMinistryCompatibilityServiceImpl`~~ | sem consumidor de producao desde `refactor/make-person-ministry-writes-official`; `PersonMinistryCompatibilityServiceImplTest` removido junto | Removida por completo em `refactor/remove-obsolete-person-ministry-write-through`, junto do teste unitario exclusivo. |
