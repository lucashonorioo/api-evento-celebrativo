...
export class EucharisticMinisterManagementComponent implements OnInit {
  ...
  submit(): void {
    ...
    const operation =
      editingMinisterId === null
        ? this.eucharisticMinisterService.create(this.createRequest())
        : this.eucharisticMinisterService.update(editingMinisterId, this.updateRequest());
  ...
  private createRequest(): EucharisticMinisterCreateRequest {
    return buildPersonMinisterialCreateRequest(this.form.getRawValue());
  }

  private updateRequest(): EucharisticMinisterUpdateRequest {
