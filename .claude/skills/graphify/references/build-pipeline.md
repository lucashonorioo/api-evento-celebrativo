# Graphify full build pipeline

This reference contains Steps 2–9 of the full/rebuild pipeline. Read it in full only when `/graphify` is performing a fresh/full build after Step 1. Preserve the same resolved input path and the pinned `$GRAPHIFY_PYTHON` interpreter established by Step 1.
...
> **graphify needs no API key for structural extraction. Never ask for one or block on one.** Code-only corpora use deterministic AST extraction and require no external LLM.

**External semantic providers are explicit opt-in.** Presence of `GEMINI_API_KEY`/`GOOGLE_API_KEY` alone is **not consent to send corpus content to Gemini**. Use `graphify.llm.extract_corpus_parallel(files, backend="gemini")` only when **both** conditions are true:

1. `GRAPHIFY_ALLOW_EXTERNAL_LLM` is explicitly `1` or `true`; and
2. `GEMINI_API_KEY` or `GOOGLE_API_KEY` is already available.

If the opt-in is absent, do not send documents, papers, images, prompts, or extracted content to Gemini even when a key exists. Use the host-agent semantic flow from Part B instead. If the opt-in is present but the key is missing, warn once and fall back to the host-agent flow without waiting for credentials. Never prompt for `ANTHROPIC_API_KEY` or `OPENAI_API_KEY` as a graphify requirement.

When the explicit Gemini opt-in is active, the default model is `gemini-3-flash-preview`; `GRAPHIFY_GEMINI_MODEL` or `--model` may override it in headless flows.
